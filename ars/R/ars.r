...
#' @param old_density_function Density to sample from
#' @param starting_points Starting points to begin adaptive sampling. Must be on either side of density maximum
#' @param bounds Bounds of density function. Must be provided when density is bounded e.g. gamma or beta 
#' @param n Number of samples to draw
#' @return n samples drawn from old_density_function
#' @examples 
#' f <- function(x) dnorm(x) #set density function to sample from
#' samples <- ars(f, n=1000) 
#' hist(samples , probability = TRUE) #plot the samples
#' curve(f, add = TRUE, col = "blue") #plot the actual function 
#' @export
